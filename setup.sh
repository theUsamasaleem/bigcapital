...
# Prints the Finqora logo once running the script.
function print_logo() {
clear
...
# Downloads /docker folder from Finqora repository
clone_github_folder() {
...
function startServices() {
    ...
    local migrator_container_id=$(docker container ls -aq -f "name=finqora-database-migration")
    ...
    if [ -n "$migrator_container_id" ]; then
        ...
        if [ $migrator_exit_code -ne 0 ]; then
            echo "Finqora Server failed to start ❌"
            stopServices
            echo
    ...
    local api_container_id=$(docker container ls -q -f "name=finqora-server")
    local idx2=0
    while ! docker logs $api_container_id 2>&1 | grep -m 1 -i "Server listening on port" | grep -q ".";
    do
        local message=">> Waiting for Finqora Server to Start"
    ...
    echo "   Finqora server started successfully ✅"
    echo ""
